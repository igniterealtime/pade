...
CandyShop.Fastpath = (function(self, Candy, $) {
	...
	var fastpathMsgCallback = function(message) 
	{
		...
		{	
			...
			{	
				...
				if (bgWindow) bgWindow.ChromeUi.openVideoWindow(room); 
			});
	...
	{				
		...
		form = form + '<input id="fastpathoffer"  type="button" value="Accept" />'
			    + '<input id="fastpathreject" type="button" value="Reject" />'
			    + '</form>'	
		
		Candy.View.Pane.Chat.Modal.show(form, true);	
		
		$('#fastpathoffer').click(function(event) 
		{
			self.acceptOffer();
		});
		
		$('#fastpathreject').click(function(event) 
		{
			self.rejectOffer();
		});		
	}	
