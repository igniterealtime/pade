(function (root, factory) {
                ...
                parseMessageForCommands: function(text) {
                    ...
                    if (command === "feed" && this.model.get("type") == "chatroom")
                    {
                        ...
                        chrome.storage.local.get(feedId, function(data)
                        {
                            ...
                            var feed = {path: match[2], url: "https://" + bgWindow.pade.server + "/apps/download?url=" + match[2]};
    ...
    var createMylinks = function(jid, id)
    {
        if (bgWindow && bgWindow.pade.collabDocs)
        {
            ...
            if (urls.length > 0)
            {
                var h5pCount = __converse.div.parentNode.getElementById(id + "-h5p-count");
                var h5pDetail = __converse.div.parentNode.getElementById(id + "-h5p-details");
                var pdfCount = __converse.div.parentNode.getElementById(id + "-pdf-count");
                var pdfDetail = __converse.div.parentNode.getElementById(id + "-pdf-details");
                var appsCount = __converse.div.parentNode.getElementById(id + "-apps-count");
                var appsDetail = __converse.div.parentNode.getElementById(id + "-apps-details");
    ...
    var createBroadcastEndpoints = function(jid, id)
    {
        ...
        {
            var count = __converse.div.parentNode.getElementById(id + "-broadcast-count");
            var detail = __converse.div.parentNode.getElementById(id + "-broadcast-details");
    ...
    var createWorkgroups = function(jid, id)
    {
        ...
        if (jid.startsWith("workgroup-"))
        {
            ...
            _converse.connection.sendIQ($iq({type: 'get', to: "workgroup." + _converse.connection.domain}).c('workgroups', {jid: _converse.connection.jid, xmlns: "http://jabber.org/protocol/workgroup"}).tree(), function(resp)
            {
                ...
                var count = __converse.div.parentNode.getElementById(id + "-wg-others-count");

                if (count) count.innerHTML = workgroups.length;

                var detail = __converse.div.parentNode.getElementById(id + "-wg-others-details");
            ...
            if (bgWindow && bgWindow.pade.fastpath[workGroup])
            {
                ...
                var queueCount = __converse.div.parentNode.getElementById(id + "-wg-queue-count");

                if (queueCount) queueCount.innerHTML = fastpath.count;

                var queueDetail = __converse.div.parentNode.getElementById(id + "-wg-queue-details");
                ...
                var chatsDetail = __converse.div.parentNode.getElementById(id + "-wg-chats-details");
                var chatsCount = __converse.div.parentNode.getElementById(id + "-wg-chats-count");
    ...
    var createContentSummary = function(view, jid, id)
    {
        ...
        if (chrome.storage)
        {
            ...
            chrome.storage.local.get('pinned', function(data)
            {
                ...
                if (data && data.pinned) pinned = data.pinned;
                const keys = Object.getOwnPropertyNames(pinned);
                const count = __converse.div.parentNode.getElementById(id + "-pinned-count");
                const detail = __converse.div.parentNode.getElementById(id + "-pinned-details");
            ...
            chrome.storage.local.get(feedId, function(data)
            {
                ...
                if (data && data[feedId])
                {
                    ...
                    const count = __converse.div.parentNode.getElementById(id + "-feed-count");
                    const detail = __converse.div.parentNode.getElementById(id + "-feed-details");
            ...
            if (getSetting("enableThreading", false))
            {
                ...
                chrome.storage.local.get(topicId, function(data)
                {
                    ...
                    if (data && data[topicId])
                    {
                        ...
                        const count = __converse.div.parentNode.getElementById(id + "-topics-count");
                        const detail = __converse.div.parentNode.getElementById(id + "-topics-details");
    ...
    var newPinnedItemElement = function(el, item, className)
    {
        ...
        item.ele.addEventListener('click', function(evt)
        {
            ...
            var elmnt = __converse.div.parentNode.getElementById("msg-" + evt.target.name);
    ...
    var newItemElement = function(el, item, className)
    {
        ...
        item.ele.addEventListener('click', function(evt)
        {
            ...
            var elmnt = __converse.div.parentNode.getElementById("msg-" + msgId);
            if (elmnt) elmnt.scrollIntoView({block: "end", inline: "nearest", behavior: "smooth"});
    ...
    var renderMedia = function (id, eleName, urls, check)
    {
        ...
        var count = __converse.div.parentNode.getElementById(id + "-" + eleName + "-count");
        var detail = __converse.div.parentNode.getElementById(id + "-" + eleName + "-details");
    ...
    var renderMeeting = function (id, meetings)
    {
        ...
        var summary = __converse.div.parentNode.getElementById(id + "-meeting-recordings");
