...
*/
  ...
  {
	console.log("remote unload", room, user);
  ...
  window.addEventListener('message', function (event) {
	...
	{
		if (event.data.json.msg && enableCursor)
		{
	...
	if (event.data.action == 'ofmeet.action.enable.cursor')
	{
		enableCursor = event.data.flag;
	}
	else
...
console.log("remote load", window.location.href);
